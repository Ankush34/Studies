...
public class access_grant_adapter extends RecyclerView.Adapter<access_grant_adapter.ViewHolder> {
    ...
    AppConfig appConfig ;
    public int position_to_block = 0;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.revoke_access.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                position_to_block = position;
                new block_user().execute();
            }
        });
    ...
    public  class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(final View itemView) {
            ...
            revoke_access = (ImageView)itemView.findViewById(R.id.revoke_permission);
        }

    }

    public class block_user extends AsyncTask<Void, Void, Void>
    {
        @Override
        protected Void doInBackground(Void... voids) {
            JsonArrayRequest request_block_user = new JsonArrayRequest(AppConfig.block_user + users.get(position_to_block).getEmail(), new Response.Listener<JSONArray>() {
                @Override
                public void onResponse(JSONArray response) {
                    try
                    {
                        ...
                        {
                          Toast.makeText(activity.getApplicationContext(),"User Blocked Successfully",Toast.LENGTH_SHORT).show();
                        }
                        else
                        {
                    ...
                    {
                        Toast.makeText(activity.getApplicationContext(),"Sorry Please Retry !!",Toast.LENGTH_SHORT).show();
                    }
                ...
                public void onErrorResponse(VolleyError error) {
                    Toast.makeText(activity.getApplicationContext(),"Sorry Error Took Place , Please Retry!! ",Toast.LENGTH_SHORT).show();
                }
            });
            request_block_user.setRetryPolicy(new DefaultRetryPolicy(
                    DefaultRetryPolicy.DEFAULT_TIMEOUT_MS,
                    DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
                    DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
            AppController.getInstance().addToRequestQueue(request_block_user);
            return null;
        }
    }
