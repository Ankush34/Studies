...
public class bestSellingAdapter extends RecyclerView.Adapter<bestSellingAdapter.viewHolder> {
    ...
    public bestSellingAdapter(ArrayList<Product> products_sent, Activity activity)
    {
    ...
    @Override
    public bestSellingAdapter.viewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.best_selling_recycler_card, parent, false);
        bestSellingAdapter.viewHolder viewHolder = new bestSellingAdapter.viewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(bestSellingAdapter.viewHolder holder, int position) {
        Product product = products.get(position);
        holder.name.setText(product.get_Name());
        holder.stock_count.setText(""+product.get_Stock()+" Units");
        holder.min_price.setText(""+product.getPrice_mop());
        holder.price.setText(""+product.getPrice_mrp());
        holder.sold_count.setText(""+product.getSold_count()+" Units");
        String[] links = product.get_link().split(",");
        Picasso.with(activity.getApplicationContext()).load(links[links.length-1])
                .placeholder( activity.getResources().getDrawable( R.drawable.weel ))
                .error( activity.getResources().getDrawable( R.drawable.nothing_found_images_product_desc))
