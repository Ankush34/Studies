...
import in.co.khuranasales.khuranasales.ledger.ledger_selection_adapter;
...
public class Buy_Now_Activity extends AppCompatActivity {
    ...
    public static int special_discount_amount = 0;
    ...
    public RelativeLayout ledger_name_recycler_view;
    public RecyclerView ledger_name_recycler;
    public ledger_selection_adapter ledger_selection_adapter;
    public ArrayList<Ledger> ledgers = new ArrayList<Ledger>();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ledger_name_recycler_view = (RelativeLayout)findViewById(R.id.ledger_name_recycler_view);
        ledger_name_recycler = (RecyclerView)findViewById(R.id.ledger_name_recycler);
        recyclerViewItemClickListener ledger_selection_listener = (view, position) -> {
            Toast.makeText(getApplicationContext(),"Clicked: "+position,Toast.LENGTH_SHORT).show();
            customer_address.setText(ledgers.get(position).getAddress());
            if(!ledgers.get(position).getPhone().equals(""))
            {
                customer_phone.setText(ledgers.get(position).getPhone());
            }
            else
            {
                customer_phone.setText(ledgers.get(position).getMobile());
            }
            customer_email.setText(ledgers.get(position).getEmail());
            customer_gst.setText(ledgers.get(position).getGSTIN());
          customer_name.setText(ledgers.get(position).getName());
            fadeOut(ledger_name_recycler_view);
            ledgers.clear();
        };
        ledger_selection_adapter = new ledger_selection_adapter(this,ledgers,ledger_selection_listener);
        ledger_name_recycler.setHasFixedSize(true);
        ledger_name_recycler.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        ledger_name_recycler.setAdapter(ledger_selection_adapter);
        ledger_selection_adapter.notifyDataSetChanged();
        ...
        customer_name.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                {
                    search_ledgers(customer_name.getText().toString());
                }
                else
                {
                    if(ledger_name_recycler_view.getVisibility() == View.VISIBLE)
                    {
                        fadeOut(ledger_name_recycler_view);
                    }
                }
        ...
        batch_selected_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                else
                {
                    ...
                    Toast.makeText(getApplicationContext(),"Selected Batch success",Toast.LENGTH_SHORT).show();
        ...
        submit_details.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                int count_of_mobile_products =  0 ;
                for(int i = 0; i < products.size();i++)
                {
                    if(products.get(i).getProduct_type().equals("Accessory"))
                    {
                        hash_status_confirm_batch.remove(i);
                    }
                    else
                    {
                        count_of_mobile_products++;
                    }
                }
                for(int i = 0 ; i < products.size();i++)
                {
                    if(products.get(i).getProduct_type().equals("Mobile"))
                    {
                ...
                if(hash_status_confirm_batch.size() != count_of_mobile_products)
                {
                ...
                if(total_batch_numbers == total_products)
                {
                    ...
                    if(applicable_discount > (mop_price_sum - ks_price_sum ))
                    {
                        Toast.makeText(getApplicationContext(),"Please give less discount, limit exceeds",Toast.LENGTH_LONG).show();
                        return;
                    }

                    String data[] = new String[6];
                    data[0] = customer_email.getText().toString();
                    data[1] = appConfig.getUser_email().trim();
                    ...
                    animator.addAnimatorListener(new Animator.AnimatorListener() {
                        ...
                        public void onAnimationEnd(Animator animator) {
                            ...
                            intent.putExtra("discount_applicable",applicable_discount + special_discount_amount);
        ...
        if(appConfig.getUserType().equals("Shop Admin") || appConfig.getUserType().equals("Admin") || appConfig.getUserType().equals("Promoter"))
        {
    ...
    public void getAllProducts()
    {
        ...
        Log.d("url",""+AppConfig.url_get_viewed+appConfig.getUser_email().trim());
        JsonArrayRequest movieReq = new JsonArrayRequest(AppConfig.url_get_viewed+appConfig.getUser_email().trim(),
                new Response.Listener<JSONArray>() {
                    @Override
                    public void onResponse(JSONArray response) {
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                ...
                                pro.setProduct_type(obj.getString("type"));
                                pro.set_link(obj.getString("links"));
                                pro.setOffer_type(obj.getString("offer_type"));
                                if(!obj.getString("offer_type").equals("NONE"))
                                {
                                ...
                                if(array.length() != 0) {
                                    ...
                                    {
                                        if(!batch_details.getString(batch_details.names().getString(j)).equals("null") && !batch_details.getString(batch_details.names().getString(j)).equals(""))
                                        {
    ...
    {
        String url = AppConfig.url_set_viewed+"?customer_email="+appConfig.getUser_email().trim()+"&id="+product.getProduct_id()+"&count="+product.get_Stock();
        Log.d("url",""+url);
    ...
    public static void update_price_card(List<Product> products,Coupon... coupons)
    {
        ...
        {
            if(!p.getOffer_type().equals("NONE"))
            {
                special_discount_amount = special_discount_amount + p.get_Stock()*p.getDiscount_offered_online_fetch();
            }
        }
        applicable_discount = (int)discount_amount;

        Log.d("APPLICABLEDISCOUNT",""+applicable_discount);
        Log.d("SPECIALDISCOUNT",""+special_discount_amount);

        text2.setText("Discount Amount:  "+(discount_amount + special_discount_amount)+" /-");
        for (int x=0;x<products.size();x++) {
            ...
            {
                init_tax = (product.getPrice_ks()*product.get_Stock()) - (product.getPrice_ks()*product.get_Stock())/((100 + product.getTax())/100);
                amount = amount + (product.getPrice_ks() * product.get_Stock()) - init_tax;
            }
            else
            {
                init_tax = (product.getPrice_mop()*product.get_Stock()) - (product.getPrice_mop()*product.get_Stock())/((100 + product.getTax())/100);
        ...
        text4.setText("Total Pay Amount  "+(Math.round(amount+tax) - discount_amount - special_discount_amount)+" /-");
        if((mop_price_sum - ks_price_sum) < discount_amount)
        {
            text2.setText("Discount amount can not be greater than "+(mop_price_sum - ks_price_sum) + "current discount: "+discount_amount);
            ...
            public void onResponse(JSONArray response) {
                ...
                for (int i = 0; i < response.length(); i++) {
                    try {
                        ...
                        e.printStackTrace();
                        ledger_name_recycler_view.setVisibility(View.INVISIBLE);
                    }
                }
                if(ledgers.size() > 0 )
                {
                    if(ledger_name_recycler_view.getVisibility() != View.VISIBLE)
                    {
                        fadeIn(ledger_name_recycler_view);
                    }
                }
                else{
                    fadeOut(ledger_name_recycler_view);
