...
public class Buy_final_activity extends AppCompatActivity {
    ...
    private RelativeLayout payment_mode_credit;
    ...
    private TextView credit_amount_confirm;
    ...
    private String invoice_number_for_bill = "";

    public EditText credit_customer_name;
    public EditText credit_customer_phone;
    public EditText customer_credit_amount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        credit_customer_name = (EditText)findViewById(R.id.customer_name);
        credit_customer_phone = (EditText)findViewById(R.id.customer_phone);
        customer_credit_amount = (EditText)findViewById(R.id.credit_amount);

        credit_customer_name.setText(getIntent().getStringExtra("customer_name"));
        credit_customer_phone.setText(getIntent().getStringExtra("customer_phone"));
        ...
        credit_amount_confirm = (TextView)findViewById(R.id.credit_amount_confirm);
        ...
        payment_mode_credit = (RelativeLayout)findViewById(R.id.payment_mode_credit);
        ...
        close_credit = (ImageView)findViewById(R.id.close_credit);
        close_credit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                payment_mode_credit.setVisibility(View.GONE);
                views_visible.remove(payment_mode_credit);
                check_views_on_delete("Credit");
                credit_amount_confirm.setText("0 /-");
            }
        });
                ...
                if (BrandSelectionCategorizationAdapter.selected_payment_modes.size() == 1) {
                    switch (BrandSelectionCategorizationAdapter.selected_payment_modes.get(0)) {
                        ...
                        case "Cards credit/debit":
                            ...
                            break;
                        case "Credit":
                            payment_mode.setImageResource(R.drawable.credit_balance_payment);
                            views_visible.add(payment_mode_credit);
                            break;
                    }
                ...
                for (int k = 0; k < BrandSelectionCategorizationAdapter.selected_payment_modes.size(); k++) {
                    ...
                    switch (mode) {
                        ...
                        case "Cash":
                            break;
                        case "Credit":
                            payment_mode_credit.setVisibility(View.VISIBLE);
                            views_visible.add(payment_mode_credit);
                            break;
                    }
                }
                ...
                popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    public boolean onMenuItemClick(MenuItem item) {
                        if (item.getItemId() == R.id.cash) {
                            ...
                            check_visibility(payment_mode_credit);
                            payment_mode.setImageResource(R.drawable.credit_balance_payment);
                            payment_mode_credit.setVisibility(View.VISIBLE);
                            views_visible.add(payment_mode_credit);
                        }
        ...
        buy_final.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                for (int i = 0; i < views_visible.size(); i++) {
                    ...
                    if(v.getId() == R.id.payment_mode_credit)
                    {
                        if(credit_customer_name.getText().toString().equals(""))
                        {
                            credit_customer_name.setError("Enter customer name");
                            entries_required = true;
                        }
                        if(credit_customer_phone.getText().toString().equals(""))
                        {
                            credit_customer_phone.setError("Enter customer phone");
                            entries_required = true;
                        }
                        if(customer_credit_amount.getText().toString().equals(""))
                        {
                            customer_credit_amount.setError("Enter amount of credit");
                            entries_required = true;
                        }
                        if(!customer_credit_amount.getText().toString().equals(""))
                        {
                            total_amount = total_amount + Integer.parseInt(customer_credit_amount.getText().toString());
                            Log.d("ViewVisible", " Credit ID: " + v.getId());
                            upload_payment_details("Credit");
                            credit_amount_confirm.setText(customer_credit_amount.getText()+" /-");
                        }
                    }
                }
                if(entries_required)
    ...
    public void set_baught() {
        ...
        Log.d("URL: ", "" + AppConfig.set_bought_with_batch_locations_and_purchase_check + appConfig.getUser_email());
        JsonArrayRequest bought_req = new JsonArrayRequest((AppConfig.set_bought_with_batch_locations_and_purchase_check + appConfig.getUser_email()+"&discount="+getIntent().getExtras().getInt("discount_applicable") +"&date="+formattedDate),
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                ...
                                invoice_number_for_bill = obj.getString("invoice_number");
        ...
        StringRequest invoice_request = new StringRequest(Request.Method.POST, AppConfig.url_generate_invoice, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                if (response != null) {
                    ...
                    intent.putExtra("invoice_number", invoice_number_for_bill);
                    ...
                    for(int i = 0;i < views_visible.size();i++)
                    {
                        ...
                        else if (views_visible.get(i).getId() == R.id.payment_mode_credit)
                        {
                            intent.putExtra("credit_amount",Integer.parseInt(customer_credit_amount.getText().toString()));
                        }
                        else
                        {
                            if(cash_payment != 0)
    ...
    public void upload_payment_details(final String payment_mode) {
        ...
        params.put("user_email",appConfig.getUser_email().trim());
        ...
        if(payment_mode.equals("Credit"))
        {
            ...
            params.put("ledger_name",credit_customer_name.getText().toString());
            params.put("ledger_phone",credit_customer_phone.getText().toString());
            params.put("credit_amount",customer_credit_amount.getText().toString());
        }
        Log.d("BUYFINALACTIVITY", params.toString());
                ...
                switch(payment_mode)
                {
                    ...
                    case "Finance":
                        Toast.makeText(getApplicationContext(),"Finance Payment Uploaded",Toast.LENGTH_LONG).show();
                         break;
                    case "Credit":
                        Toast.makeText(getApplicationContext(),"Credit Payment Uploaded",Toast.LENGTH_LONG).show();
                        break;
                }
            }
        }, new Response.ErrorListener() {
...
class check_availablity extends AsyncTask<Void, Void , Void >
{
    ...
    protected Void doInBackground(Void... voids) {
        ...
        JsonArrayRequest check_final_availability = new JsonArrayRequest(AppConfig.check_availability_order + appConfig.getUser_email().trim(), new Response.Listener<JSONArray>() {
            @Override
            public void onResponse(JSONArray response) {
                try {
        ...
        check_final_availability.setRetryPolicy(new DefaultRetryPolicy(
                DefaultRetryPolicy.DEFAULT_TIMEOUT_MS,
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
                DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
