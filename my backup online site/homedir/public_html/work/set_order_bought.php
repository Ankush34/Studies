...
$discount_applicable = isset($_GET['discount']) && $_GET['discount'] != '' ? $_GET['discount'] : 0;
...
if(mysqli_num_rows($result)>0){
	...
	while ($row = mysqli_fetch_array($result))
	{
		 ...
		 $sql2 = "update khurana_sales_orders set sales_order_invoice_number = $invoice_number ,sales_order_discount = $discount_applicable , sales_order_status = 'ORDERED',sales_order_date = '$date_of_order' where sales_order_customer_email = '$email' and sales_order_product_name = '$product_name' and  sales_order_status = 'VIEWED'";
		 mysqli_query($conn,$sql2);
		
		$batch_selected = $row["batches_selected"];
		$batches_selected_array = explode(",",$batch_selected);
		$name_of_product = $row["sales_order_product_name"];
		$price_of_product = $row["sales_order_price"];
		$sql_batch_id = "select BatchId from khurana_sales_stock where Name = '$name_of_product'";
		$result_batch_id = mysqli_query($conn, $sql_batch_id);
		$row_batch_id = mysqli_fetch_assoc($result_batch_id);
		$batch_id = $row_batch_id["BatchId"];
		$batch_query = "select * from batch_numbers where id = $batch_id";
		$result_batch = mysqli_query($conn, $batch_query);
		$row_batch_complete = mysqli_fetch_assoc($result_batch);
		$fields = mysqli_fetch_fields($result_batch);
		...
		else if($fields[$i]->name == "product_id")
		{
		continue;
		}
		if($row_batch_complete[$fields[$i]->name] != null)
		{
		$batches_present = $row_batch_complete[$fields[$i]->name];
		$batches_present_array = explode(",", $batches_present);
		$array = array_diff($batches_present_array, $batches_selected_array);
		$batches_now_after_update = join(",",$array);
		$column = $fields[$i]->name;
		$sql_batch_update = "update batch_numbers set `$column` = '$batches_now_after_update' where id = $batch_id";
		$result_update = mysqli_query($conn, $sql_batch_update) or die(mysqli_error($conn));
		}
		...
		$sql3 = "update khurana_sales_stock set Stock = Stock - $row[sales_order_quantity] where Name = '$product_name'";
	        ...
	        $sql4 = "update khurana_sales_access set outstanding_amount = outstanding_amount + $price_of_product where user_email = '$email'";
