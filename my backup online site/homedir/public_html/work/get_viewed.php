...
$i=0;
    while ($row = mysqli_fetch_array($result)) {
        ...
        $product[$i]=$row["sales_order_product_name"];
         ...
         {
           $offer_id = $row["offer_description_id"];
           $sql = "select * from offer_desc_cart_product where offer_id = '$offer_id' ";
           $result_offer = mysqli_query($conn, $sql);
           $row_result_offer = mysqli_fetch_assoc($result_offer);
           $offer_type[$i] = $row_result_offer["offer_type"];
           $discount_amount[$i] = $row_result_offer["discount_amount"];
           $item_count[$i] = $row_result_offer["item_count"];
         }
...
if (mysqli_num_rows($result2)>0) {
    while ($row = mysqli_fetch_array($result2)) {
        ...
        $product2["product_tax"] = $row["product_tax_percent"];
        $product2["batch_selected"] = $batches_selected[$k];
        $product2["offer_type"] = $offer_type[$k];
        if(strpos($product2["name"], 'Mobile') !== false)
        {
        $product2["type"] = "Mobile";
        }
        else
        {
        $product2["type"] = "Accessory";
        }
        if($product2["offer_type"] != "NONE")
        {
          $product2["discount_amount"] = $discount_amount[$k];
          $product2["item_count"] = $item_count[$k];
        }
